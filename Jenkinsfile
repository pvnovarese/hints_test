    ...
    stage('Analyze Image with anchorectl') {
      steps {
        ...
        sh """
          ...
          ${HOME}/.local/bin/anchorectl image add --no-auto-subscribe --wait --force --dockerfile ./Dockerfile --from registry --annotation build_tool=jenkins --annotation scan_type=distributed-registry ${IMAGE}
          #
          ${HOME}/.local/bin/anchorectl image add --no-auto-subscribe --wait --force --dockerfile ./Dockerfile --from docker --annotation build_tool=jenkins --annotation scan_type=distributed-docker ${IMAGE}
          #
